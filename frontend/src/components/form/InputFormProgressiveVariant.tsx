...
import type { IvfWillingness } from "../../api/types";
...
import { PillToggle, type Props } from "./InputForm";
...
export default function InputFormProgressiveVariant({ form, onChange, onSubmit, loading }: Props) {
          ...
          {step === 1 && (
            <div>
              <button
                type="button"
                onClick={() => setStep(2)}
                className="w-full rounded-md bg-stone-800 px-4 py-3 text-sm font-semibold text-white hover:bg-stone-900 transition-colors"
              >
                Continue
              </button>
            </div>
